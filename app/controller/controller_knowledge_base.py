...
def create_database(name: str):
    if name in database_container:
        raise HTTPException(
            status_code=409, detail="database is already created"
        )
    else:
...
def delete_database(name: str):
    if name not in database_container:
        raise HTTPException(
            status_code=404, detail="database is not created"
        )
    else:
...
def similarity_search(name: str, query: str):
    ...
    for doc, score in results_with_scores:
        parsed_result = {
            ...
            "score": str(score) # we need to convert score to a foramt that fastAPI can parse
        }
